...
class SurveyGizmo {
  ...
  getResponses() {
    return new Promise((resolve, reject) => {
      const url = apiKey ? `${sgUrl}/${sgSurveyId}/surveyresponse?api_token=${apiKey}&filter[field][1]=status&filter[operator][1]==&filter[value][1]=Complete` : `${config.baseUrl}:${config.applicationPort}/surveyresponse.json`;
