...
const excludesRequestRoute = _.curry((routes, req) => !routes.some(route => req.path && req.path === route))

const intercept = _.curry((predicate, middleware) =>
  (req, res, next) => predicate(req) ?
    middleware(req, res, next)
    :
    next())

const isNotWhitelisted = excludesRequestRoute(whitelistRoutes)
const skipWhitelistedRoutes = intercept(isNotWhitelisted)
